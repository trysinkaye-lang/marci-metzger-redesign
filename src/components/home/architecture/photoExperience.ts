...
export function createPhotoExperience(section: HTMLElement, signal: AbortSignal) {
  ...
  const update = (rawProgress: number) => {
    ...
    /* Keep full-viewport photos very close to native scale so they stay crisp. */
    const approach = range(progress, .06, .26)
    section.style.setProperty('--backplate-scale', mix(1.035, 1.005, approach).toFixed(4))
    ...
    section.style.setProperty('--threshold-scale', mix(1.035, 1, thresholdIn).toFixed(4))
    ...
    section.style.setProperty('--photo-interior-scale', mix(1.045, 1.005, range(progress, .24, .63)).toFixed(4))
    section.style.setProperty('--photo-interior-x', `${mix(1.2, 0, range(progress, .30, .58)).toFixed(2)}%`)
    ...
    section.style.setProperty('--photo-outdoor-scale', mix(1.04, 1.005, range(progress, .62, .82)).toFixed(4))
    section.style.setProperty('--photo-outdoor-y', `${mix(1, 0, range(progress, .62, .80)).toFixed(2)}%`)
    ...
    section.style.setProperty('--photo-community-scale', mix(1.03, 1, range(progress, .80, .95)).toFixed(4))
